...
class TestFfq(TestMixin, TestCase):
    ...
    def test_parse_run(self):
        self.maxDiff = None
        with mock.patch(
            "BSM.BSM.get_files_metadata_from_run"
        ) as get_files_metadata_from_run, mock.patch(
            "BSM.BSM.ncbi_fetch_fasta"
        ) as ncbi_fetch_fasta, mock.patch(
            "BSM.BSM.parse_ncbi_fetch_fasta"
    ...
    def test_parse_experiment_with_run(self):
        ...
        self.assertEqual(
            ...
            bsm.parse_experiment_with_run(soup, 10),
        )
    ...
    def test_ffq_run(self):
        with mock.patch("BSM.BSM.get_xml") as get_xml, mock.patch(
            "BSM.BSM.parse_run"
        ) as parse_run:
            run = mock.MagicMock()
            parse_run.return_value = run
            self.assertEqual(run, bsm.ffq_run("SRR8426358"))
            get_xml.assert_called_once_with("SRR8426358")

    def test_ffq_study(self):
        with mock.patch("BSM.BSM.get_xml") as get_xml, mock.patch(
            "BSM.BSM.parse_study"
        ) as parse_study, mock.patch("BSM.BSM.ffq_sample") as ffq_sample, mock.patch(
            "BSM.BSM.get_samples_from_study"
    ...
    def test_ffq_experiment(self):
        with mock.patch("BSM.BSM.get_xml") as get_xml, mock.patch(
            "BSM.BSM.parse_experiment_with_run"
    ...
    #     BSM.ffq_links([('GSE', 'GSE112570')], 'ftp')
    ...
    #     BSM.ffq_links([('SRS', 'SRS4629239')], 'ftp')  # Call function.
    #     sys.stdout = sys.__stdout__
    #     self.assertEqual(
    #         capturedOutput.getvalue(),
    ...
    #     capturedOutput = io.StringIO()
    #     sys.stdout = capturedOutput
    #     BSM.ffq_links([('GSM', 'GSM3396164')], 'AWS')
    #     sys.stdout = sys.__stdout__
    #     self.assertEqual(
    #         capturedOutput.getvalue(),
    ...
    #     capturedOutput = io.StringIO()
    #     sys.stdout = capturedOutput
    #     BSM.ffq_links([('SRR', 'SRR8327928')], 'GCP')
    #     sys.stdout = sys.__stdout__
    #     self.assertEqual(
    #         capturedOutput.getvalue(),
    ...
    #     capturedOutput = io.StringIO()
    #     sys.stdout = capturedOutput
    #     BSM.ffq_links([('SRX', 'SRX4063411')], 'NCBI')
    #     sys.stdout = sys.__stdout__
    #     self.assertEqual(
    #         capturedOutput.getvalue(),
    ...
    def test_ffq_doi(self):
        with mock.patch("BSM.BSM.get_doi") as get_doi, mock.patch(
            "BSM.BSM.search_ena_title"
        ) as search_ena_title, mock.patch("BSM.BSM.ffq_study") as ffq_study:

            get_doi.return_value = {"title": ["title"]}
            search_ena_title.return_value = ["SRP1"]
            self.assertEqual([ffq_study.return_value], bsm.ffq_doi("doi"))
    ...
    def test_ffq_doi_no_title(self):
        with mock.patch("BSM.BSM.get_doi") as get_doi, mock.patch(
            ...
            self.assertEqual([ffq_gse.return_value], bsm.ffq_doi("doi"))
            get_doi.assert_called_once_with("doi")
            search_ena_title.assert_called_once_with("title")
