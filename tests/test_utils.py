...
import BSM.utils as utils
from BSM.config import (
    CROSSREF_URL,
    ENA_SEARCH_URL,
    ENA_URL,
    GSE_SEARCH_URL,
...
class TestUtils(TestMixin, TestCase):
    def test_cached_get(self):
        with mock.patch("BSM.utils.requests") as requests:
            self.assertEqual(requests.get().text, utils.cached_get())

    def test_get_xml(self):
        with mock.patch("BSM.utils.cached_get") as cached_get:
    ...
    def test_get_gsm_search_json(self):
        with mock.patch("BSM.utils.ncbi_search") as ncbi_search:
    ...
    def test_ncbi_summary(self):
        with mock.patch("BSM.utils.requests.get") as get:
    ...
    def test_geo_id_to_srps(self):
        with mock.patch("BSM.utils.ncbi_summary") as ncbi_summary:
    ...
    def test_geo_id_to_srps_bioproject(self):
        with mock.patch("BSM.utils.ncbi_summary") as ncbi_summary, mock.patch(
            "BSM.utils.ncbi_search"
        ) as ncbi_search, mock.patch("BSM.utils.ncbi_link") as ncbi_link:
    ...
    def test_gse_to_gsms(self):
        with mock.patch(
            "BSM.utils.get_gse_search_json"
        ) as get_gse_search_json, mock.patch("BSM.utils.ncbi_summary") as ncbi_summary:
    ...
    def test_gsm_to_srx(self):
        with mock.patch(
            "BSM.utils.get_gsm_search_json"
        ) as get_gsm_search_json, mock.patch("BSM.utils.ncbi_summary") as ncbi_summary:
