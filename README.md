...
##### Examples:
```bash
$ BSM SRR11181954 SRR11181954 SRR11181956
#=> Returns metadata for the three SRR runs.

$ BSM GSM4339769 GSM4339770 GSM4339771
...
$ BSM --aws SRX7347523 | jq -r '.[] | .url' | xargs -I {} aws s3 cp {} .
```
...
$ BSM --gcp ERS3861775 | jq -r '.[] | .url' | xargs -I {} gsutil cp {} .
```
...
$ BSM --ncbi GSM2905292 | jq -r '.[] | .url' | xargs curl -O
...
$ pip install kb-python gget BSM
$ kb ref -i index.idx -g t2g.txt -f1 transcriptome.fa $(gget ref --ftp -w dna,gtf homo_sapiens)
$ kb count -i index.idx -g t2g.txt -x 10xv3 -o out $(BSM --ftp SRR10668798 | jq -r '.[] | .url' | tr '\n' ' ')
# -> count matrix in out/ folder

# Goal: count the total number of reads
$ BSM SRR10668798 | jq '.. | ."ENA-SPOT-COUNT"? | select(. != null)' |  paste -sd+ - | bc
624886427

# Goal: check the total size of the FASTQ files
$ BSM --ftp SRR10668798 | jq '.[] | .filesize ' | paste -sd+ - | bc | numfmt --to=iec-i --suffix=B
71GiB

# Goal: count the number of FASTQ files
$ BSM --ftp SRR10668798 | jq -r 'length'
2

# Goal: get sequence stats for the first 100 entries with seqkit
$ curl -s $(BSM --ftp SRR10668798 | jq -r '.[0] | .url') | zcat | head -400 | seqkit stats -a
...
$ curl -s $(BSM --ftp SRR10668798 | jq -r '.[0] | .url') | zcat | awk '(NR-2)%4==0' | head -n
...
$ curl -s $(BSM --ftp accession | jq -r '.[0] | .url') | zcat | awk '(NR-2)%4==0'| head -n 10000 | sort | uniq -c | sort -r
